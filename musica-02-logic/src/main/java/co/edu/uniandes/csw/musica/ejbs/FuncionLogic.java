...
public class FuncionLogic {
    ...
    public FuncionEntity agregarEntrada(EntradaEntity entity, Long id) throws BusinessLogicException {
        FuncionEntity fun = funcionPersistence.find(id);
        int ent = fun.getEntradasDisponibles();
        if(ent > 0){
            fun.setEntradasDisponibles(ent--);
            return funcionPersistence.agregarEntrada(entity.getId(), id);
        } 
        else {
